...
3. `wiki/index.md` (if exists) — overview of accumulated knowledge; read to understand what research is already in the wiki
4. `decisions.md` — full decision log
5. `discovery/checkpoint-1.md` (if exists) — CP1 status and key outputs
6. `design/checkpoint-2.md` (if exists) — CP2 status and key outputs
7. Most recent `build/checkpoint-3-M*.md` file (if any exist) — most recent CP3 status (files named `checkpoint-3-M1.md`, `checkpoint-3-M2.md`, etc.)
8. `build/implementation-plan.md` (if exists) — current milestone and task progress
